...
if [ -r ./import-hsdb-dump-to-mariadb.sh ]; then
    ...
    echo "$(date) ERROR ./import-hsdb-dump-to-mariadb.sh nicht gefunden. Exit 1"
    exit 1
fi

SCRIPT_HOME=$(cd "$(dirname "$0")" && pwd)
DUMP_DIR=$SCRIPT_HOME/../HSP-DB-dump
echo "$(date) INFO Running from $SCRIPT_HOME"

if test -f "$DUMP_DIR"/HSP-DB-dump_Teil-1.zip; then
    echo "$(date) INFO Merging dump-files..."
    ...
    echo "$(date) INFO Merge finished."
fi

DB_NAME="hsdb"
echo "$(date) INFO Update Database: $DB_NAME"
...
do
    ...
    if test -f "$DUMP_DIR/$file"; then
        ...
        echo "$(date) ERROR $DUMP_DIR/$file nicht gefunden. Skipping..."
    fi
done